...
SELECT
	...
	"date_last"
FROM "trip"
ORDER BY ("date_last"::DATE - "date_first"::DATE) + 1
LIMIT 1;
...
FROM "trip"
WHERE (
	SELECT MIN("date_last"::DATE - "date_first"::DATE)
	FROM "trip") = ("date_last"::DATE - "date_first"::DATE);
...
FROM "trip"
WHERE 
   (EXTRACT(MONTH FROM "date_first") = 2 AND 
	EXTRACT(YEAR FROM "date_first") = 2020)
	OR 
   (EXTRACT(MONTH FROM "date_first") = 3 AND 
    EXTRACT(YEAR FROM "date_first") = 2020)
...
SELECT
	"name",
	SUM("per_diem" * (("date_last"::DATE - "date_first"::DATE) + 1)) AS "Сумма"
FROM "trip"
GROUP BY "name"
HAVING COUNT("name") > 3
ORDER BY "Сумма" DESC;
